...
export const getOrganisator = async (req: Request, res: Response) => {
  try {
    ...
    if (!organisator) {
      return notFound(
        res,
        `Data organisator dengan id ${validate.data} tidak ditemukan`
      );
    }
...
export const updateOrganisator = async (req: Request, res: Response) => {
  try {
    ...
    const validateData = await organisatorUpdatableSchema
      .partial()
    ...
    if (!isExists) {
      return notFound(
        res,
        `Data organisator dengan id ${validateId.data} tidak ditemukan`
      );
    }
    ...
    if (req.user?.role === "panitia") {
      logging(
        "LOGS",
        `Panitia dengan NIM ${req.user.data.nim} mengupdate data organisator dengan NIM ${organisator.nim}`,
        organisator
      );
    }
...
export const deleteOrganisator = async (req: Request, res: Response) => {
  try {
    ...
    if (req.user?.role === "panitia") {
      logging(
        "LOGS",
        `Panitia dengan NIM ${req.user.data.nim} menghapus data organisator dengan NIM ${organisator.nim}`,
        organisator
      );
    }
    return success(
      res,
      `Data organisator dengan id ${validate.data} berhasil terhapus`
    );
  } catch (err) {
