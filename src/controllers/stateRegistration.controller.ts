...
export const getStateRegistration = async (req: Request, res: Response) => {
    try {
        ...
        return success(res, "Successfully fetched state registration data", stateRegistration);
    } catch (err) {
...
export const addStateRegistration = async (req: Request, res: Response) => {
    try {
        ...
        // Check day ID
        const validateDayID = mahasiswa.StateRegistration.some(state => state.state.dayId === selectedState.dayId)
        if (validateDayID) {
            return conflict(res, "Mahasiswa sudah mendaftar state pada hari yang sama");
        }
        ...
        logging("LOGS", `${mahasiswa.nim} menambahkan state ${selectedState.name}`, stateRegistration);
        return success(res, "Berhasil menambahkan State Mahasiswa",stateRegistration);
    } catch (err) {
        logging("ERROR", "Error when trying to fetch state registration data", err);
        return internalServerError(res);
    }
...
export const deleteStateRegistration = async (req: Request, res: Response) => {
    try {
        ...
        logging("ERROR", "Error when trying to delete state registration data", err);
        return internalServerError(res);
    }
